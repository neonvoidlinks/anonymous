...
// --- PROFILE UI (MINIMALIST WHITE BORDER) ---
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (urlParams && urlParams !== "" && !urlParams.startsWith("id=")) {
        ...
        // Automatic anonymous login if no user exists
        signInAnonymously(auth).catch(() => window.showScreen('screen-landing'));
    }
});
...
window.checkAge = () => {
    ...
    if (year && (currentYear - year >= 13)) {
        window.showScreen('screen-username');
    } else {
        window.showToast("You must be 13+ to enter. 🔞");
    }
};
...
window.toggleTab = (tab) => {
    ...
    document.getElementById('t-inbox').classList.toggle('active-tab', isInbox);
    document.getElementById('t-share').classList.toggle('active-tab', !isInbox);
    document.getElementById('view-inbox').style.display = isInbox ? 'block' : 'none';
    document.getElementById('view-share').style.display = isInbox ? 'none' : 'block';
    
    // Clear unread badge when opening inbox
    if (isInbox) {
        document.getElementById('notif-badge').classList.remove('active-badge');
        updateDoc(doc(db, "users", auth.currentUser.uid), { hasUnread: false });
    }
};
